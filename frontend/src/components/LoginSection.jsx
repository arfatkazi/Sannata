...
export default function LoginSection({ onSwitch }) {
  ...
  const handleLogin = async () => {
    try {
      ...
      const res = await apiClient.post("/auth/standard-login", {
    ...
    <div>
      ...
      >
        <div>
          <label
            htmlFor="login-id"
            className="block text-sm/6 font-bold text-gray-900 cursor-pointer mr-2"
          >
            username or email
          </label>
          <Input
        ...
        <div>
          <div className="flex items-center gap-2">
            <label
              htmlFor="password"
              className="block text-sm/6 font-medium text-gray-900 cursor-pointer mr-2"
            >
              Password
            </label>
      ...
      <div className="mt-4 hidden text-xs">
        ...
        acknowledge our <a href="#">Privacy Policy</a> and
